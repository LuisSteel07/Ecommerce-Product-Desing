...
export default function Product({product, setCount}:ProductComponentProps){
    ...
    return (
        <section className="dark:text-white flex flex-col gap-8 md:w-[360px] md:mt-8 m-8">
            <div>
                <h6 className="dark:text-white/60 font-bold text-black/60 text-[12px]">{product.name}</h6>
                <h1 className="font-bold md:text-4xl text-2xl">{product.title}</h1>
            </div>
            <p className="dark:text-white/60 text-black/60 text-[14px]">{product.description}</p>
            <div className="flex md:flex-col flex-row justify-between md:items-start items-center">
                ...
                <p className="text-base dark:text-white/60 text-black/60 line-through">${product.price}</p>
            </div>
