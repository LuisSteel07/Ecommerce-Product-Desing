            ...
            <div className="flex w-full items-center justify-between relative">
                <div className="z-50 lg:hidden flex justify-between absolute w-full">
                    <span className="w-8 h-8 rounded-full bg-white flex items-center justify-center" onClick={() => handle_change_image(position-1)}>
                        <Image src="/icon-previous.svg" alt="icon previous" className="p-2"/>
                    </span>
                    <span className="w-8 h-8 rounded-full bg-white flex items-center justify-center" onClick={() => handle_change_image(position+1)}>
                        <Image src="/icon-next.svg" alt="icon previous" className="p-2"/>
                    </span>
                </div>
                <Image src={image} alt="product photo" className="z-10 md:rounded-2xl rounded-none md:w-[360px] w-full"/>
            </div>
        </section>
    )
}